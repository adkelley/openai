...
pub fn with_limit(config: FileListParams, limit: Int) -> FileListParams {
...
pub fn with_order(config: FileListParams, order: String) -> FileListParams {
...
pub fn with_after(config: FileListParams, after: String) -> FileListParams {
...
pub fn expires_after(seconds: Int) -> Option(ExpiresAfter) {
...
pub fn create(
  ...
  let req =
    base_req
    |> request.prepend_header("Authorization", "Bearer " <> client.config)
...
) -> Result(FileObjectPage, OpenAIError) {
...
pub fn content(client: Client, file_id: String) -> Result(BitArray, OpenAIError) {
